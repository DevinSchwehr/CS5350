...
def Recursive_ID3(data, attributes, total_entropy, defined_depth, decider, attribute_size):
    if defined_depth == 0:
    ...
    rand_attributes  = Pick_Random_Attributes(attributes,attribute_size)
    if decider == 1:
        root_node, new_error = Get_Root_Node_Entropy(data, rand_attributes, total_entropy)
    if decider == 2:
        root_node, new_error = Get_Root_Node_Gini(data, rand_attributes, total_entropy)
    if decider == 3:
        root_node, new_error = Get_Root_Node_Majority(data, rand_attributes, total_entropy)
    for value in attribute_value_dictionary[root_node.attribute]:
        ...
        if length == 0:
            ...
            root_node.next[value] = Recursive_ID3(value_subset,new_attributes, new_error, new_depth, decider,attribute_size)
    return root_node

def Pick_Random_Attributes(attributes, attribute_size):
    #If there are less attributes than the partition size, just return the attributes (i.e. 5 attributes for 6 partition)
    if len(attributes) < attribute_size:
        return attributes
    i = 0
    output = []
    # print('picking random attributes with ' + str(len(attributes)) + ' attributes available')
    while i < attribute_size:
        current_rand = random.randint(0,len(attributes)-1)
        if attributes[current_rand] not in output:
            output.append(attributes[current_rand])
            i +=1
    # print('picked random attributes')
    return output
